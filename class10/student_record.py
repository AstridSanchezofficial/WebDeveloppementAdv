class studentRecord:
    ...
    ##Creating read only property academic_status
    @property
    def academic_status(self):

        if self.__gpa>=3.5:
            return f"{self.name} is a Honor Student"
        elif self.__gpa>=2.0:
            return f"{self.name} is a Good standing Student"
        else:
            return f"{self.name} is a Student at risk"
